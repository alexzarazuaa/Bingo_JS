...
const PubSub = require('./public/core/pubSub.js');
...
io.on('connect', (socket) => {
  ...
  socket.on('join', playerName => {
    ...
    /**
     * CHECKS LINEA AND BINGO
     */

    socket.on('LINEA', player => {
      player.checksum = null;
      io.sockets.in(player.idplay).emit('LINEA', player);
    })


    socket.on('BINGO', player => {
      player.checksum = null;
      io.sockets.in(player.idplay).emit('BINGO', player);
    ...
    pubSub.subscribe("game_over", (data) => {
      io.sockets.in(game.id).emit('game_over', data);
    });
